  ...
  return (
    <div className="flex flex-col md:flex md:flex-row gap-6 h-full text-gray-900 pb-8">
      ...
      <div className="w-full md:w-[300px] bg-white border border-gray-200 rounded-[14px] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex flex-col overflow-hidden md:shrink-0">
         <div className="p-5 border-b border-gray-200 flex flex-col gap-4">
            <h2 className="text-[16px] font-bold text-gray-900 flex items-center justify-between">
              Tenant 관리
              <span className="text-gray-400 text-xs font-semibold px-2 border border-gray-200 rounded-full">Total 12</span>
            </h2>
         ...
         <div className="flex-1 overflow-y-auto">
            {mockTenants.map((tenant, idx) => (
              <div 
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer flex justify-between items-center transition-all ${
                  activeTenantIdx === idx 
              ...
              >
                 <div className="flex flex-col gap-1">
                   <span className={`font-extrabold text-[14px] ${activeTenantIdx === idx ? 'text-primary-700' : 'text-gray-800'}`}>{tenant.name}</span>
                   <span className="text-[12px] font-semibold text-gray-500">프로젝트 {tenant.subtenants.length}개</span>
                 </div>
                 <button className="text-gray-400 hover:text-gray-900 p-1" onClick={e => e.stopPropagation()}><MoreVertical size={16}/></button>
              </div>
            ))}
         </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-5 overflow-hidden">
         {/* 상세 헤더 */}
         <div className="bg-white border border-gray-200 rounded-[14px] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] p-6">
            <div className="flex justify-between items-start">
               <div>
                  <h1 className="text-2xl font-extrabold text-gray-900 mb-3 flex items-center gap-2">
                    <Building2 className="text-gray-400"/> {t.name}
                  </h1>
                  <div className="flex items-center gap-4 text-sm font-medium text-gray-600">
                    <span className="bg-gray-100 px-3 py-1 rounded-md flex items-center gap-1.5"><Users size={14}/> 계약자: {t.contractorEmail}</span>
                    <span className="bg-gray-100 px-3 py-1 rounded-md flex items-center gap-1.5"><Calendar size={14}/> 계정 생성: {t.createdAt}</span>
                    <span className="bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1 rounded-md flex items-center gap-1.5 ml-2">계약: {t.contract?.startDate} ~ {t.contract?.endDate}</span>
                  </div>
               </div>
               <button onClick={handleEditClick} className="flex items-center gap-1.5 px-4 py-2 border-2 border-gray-200 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm">
                 <Edit size={16}/> 상세 편집
               </button>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-white border border-gray-200 rounded-[14px] p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex items-center gap-4">
               ...
               <div>
                 <div className="text-[12px] font-bold text-gray-500">프로젝트 수 (Subtenant)</div>
                 <div className="text-[24px] font-extrabold text-gray-900">{t.subtenants.length}<span className="text-gray-400 text-sm ml-1 font-semibold">개</span></div>
               </div>
            </div>
            <div className="bg-white border border-gray-200 rounded-[14px] p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex items-center gap-4">
               <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center shrink-0"><Server size={24}/></div>
               <div>
                 <div className="text-[12px] font-bold text-gray-500">생성된 GPU 인스턴스</div>
                 <div className="text-[24px] font-extrabold text-gray-900">{t.subtenants.length > 0 ? '12' : '0'}<span className="text-gray-400 text-sm ml-1 font-semibold">대</span></div>
               </div>
            </div>
            <div className="bg-white border border-gray-200 rounded-[14px] p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex items-center gap-4">
               <div className="w-12 h-12 bg-primary-50 text-primary-600 rounded-full flex items-center justify-center shrink-0"><Cpu size={24}/></div>
               <div>
                 <div className="text-[12px] font-bold text-gray-500">전체 할당된 GPU</div>
                 <div className="text-[24px] font-mono font-extrabold text-gray-900">{t.contract?.gpu?.quantity || 0}<span className="text-gray-400 text-sm ml-1 font-semibold">장</span></div>
               </div>
            </div>
         </div>
         ...
         <div className="flex-1 bg-white border border-gray-200 rounded-[14px] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex flex-col overflow-hidden">
            <div className="p-5 border-b border-gray-200 flex justify-between items-center">
               <h3 className="font-extrabold text-gray-900 text-[15px]">프로젝트 목록</h3>
               <div className="flex gap-3">
                 <div className="relative">
                   <input type="text" placeholder="프로젝트 검새" className="w-[200px] border border-gray-200 rounded-lg pl-9 pr-3 py-1.5 text-[13px] bg-gray-50" />
                   <Search size={14} className="absolute left-3 top-2.5 text-gray-400" />
                 </div>
                 <button onClick={() => setSubtenantModal(true)} className="flex items-center gap-1.5 px-4 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-[13px] font-bold shadow-sm shadow-emerald-500/30 transition-colors">
               ...
               <table className="w-full text-left">
                  ...
                  <tbody>
                    {t.subtenants.length === 0 ? (
                      <tr><td colSpan={8} className="text-center py-16 text-gray-400 text-sm font-medium">등록된 프로젝트가 없습니다.</td></tr>
                    ) : (
                      t.subtenants.map(sub => (
                        <tr key={sub.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                           ...
                           <td className="px-5 py-4 font-mono text-xs font-semibold text-gray-500">{sub.id}</td>
                           <td className="px-5 py-4 text-xs font-bold text-gray-600">{sub.products}</td>
                           <td className="px-5 py-4 text-xs font-medium text-gray-500">{sub.startDate}<br/>~ {sub.endDate}</td>
                           <td className="px-5 py-4 text-[13px] font-medium text-gray-700">{sub.pm}</td>
                           <td className="px-5 py-4 text-center text-[13px] font-bold text-gray-800">{sub.memberCount}명</td>
                           <td className="px-4 py-4"><button className="text-gray-400 hover:text-gray-900 hover:bg-gray-100 p-1.5 rounded"><MoreVertical size={16}/></button></td>
                        </tr>
                      ))
                    )}
                  </tbody>
