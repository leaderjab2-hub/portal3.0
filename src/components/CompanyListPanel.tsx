...
export default function CompanyListPanel({ companies, activeIndex, onCompanyClick, title = '회사 목록' }: CompanyListPanelProps) {
  ...
  return (
    <div className="w-[260px] bg-white border border-gray-200 rounded-[10px] flex flex-col overflow-hidden h-full shrink-0 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.02)]">
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredCompanies.length > 0 ? (
          filteredCompanies.map((c) => {
            // Because we filter, the index passed to onCompanyClick should be the ORIGINAL index in `companies`.
